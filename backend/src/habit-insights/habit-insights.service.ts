...
export class HabitInsightsService {
  ...
  constructor(
    ...
    this.mlApiUrl =
      this.configService.get<string>('ML_API_URL') || 'http://localhost:5000';
  }
  ...
  async generateInsight(
    dto: GetHabitInsightDto & { userId: string },
    ...
    const foodLogs = rawLogs.map((log) => {
      ...
      const nutrients = items.reduce((acc, item) => {
        ...
        return {
          calories:
            (acc.calories || 0) + (nutrientData.calories || 0) * multiplier,
          protein:
            (acc.protein || 0) + (nutrientData.protein || 0) * multiplier,
          ...
          cholesterol:
            (acc.cholesterol || 0) +
            (nutrientData.cholesterol || 0) * multiplier,
        };
      }, {} as any);
    ...
    const uniqueDates = new Set(
      foodLogs.map(
        (log) => new Date(log.created_at).toISOString().split('T')[0],
      ),
    ).size;
  ...
  private calculateDateRange(dto: GetHabitInsightDto): {
    startDate: Date;
    endDate: Date;
  } {
  ...
  private detectVegetableConsistency(foodLogs: any[]): HabitPatternDto | null {
    ...
    sortedDates.forEach((date) => {
      ...
      const hasVegetable = logs.some((log) => {
        ...
        const hasVegCategory = log.categories?.some(
          (cat: string) =>
            cat.toLowerCase().includes('vegetable') ||
            cat.toLowerCase().includes('sayur'),
        );
        const hasVegKeyword = vegetableKeywords.some((keyword) =>
  ...
  private detectSugarPattern(foodLogs: any[]): HabitPatternDto | null {
    ...
    const weekendSugarLogs = weekendLogs.filter((log) => {
      ...
      const hasSugarKeyword = sugarKeywords.some((keyword) =>
        text.includes(keyword),
      );
    ...
    const sugarPercentage =
      (weekendSugarLogs.length / weekendLogs.length) * 100;
